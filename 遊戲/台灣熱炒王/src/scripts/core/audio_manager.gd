...
## 依名稱載入 BGM；先嘗試 .wav，找不到再嘗試 .ogg
## name: 檔案名稱（不含副檔名），例如 "main_theme"
func _load_bgm(name: String) -> AudioStream:
	...
	push_warning("[AudioManager] 找不到 BGM 檔案：%s（.wav/.ogg 均不存在）" % name)
	return null


## 依名稱播放 BGM（自動偵測 .wav 或 .ogg）
## name: 檔案名稱（不含副檔名），例如 "main_theme"
func play_bgm_by_name(name: String) -> void:
