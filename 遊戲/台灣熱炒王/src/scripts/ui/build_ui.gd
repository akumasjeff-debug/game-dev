...
func _create_exit_button() -> void:
	## 在右上角動態建立「╳ 離開」按鈕，掛在自身 CanvasLayer 上。
	## 預設 visible = false，進入建造模式時才顯示。
	var canvas: CanvasLayer = CanvasLayer.new()
	canvas.name = "BuildExitButtonLayer"
	canvas.layer = 20  # 高於其他 build UI，確保按鈕在最上層
	add_child(canvas)

	_exit_button = Button.new()
	_exit_button.name = "ExitBuildButton"
	_exit_button.text = "╳ 離開"
	_exit_button.position = Vector2(440, 4)
	_exit_button.size = Vector2(36, 14)
	_exit_button.visible = false

	# 設定按鈕樣式（背景 #FF2D55，文字白色）
	var style: StyleBoxFlat = StyleBoxFlat.new()
	style.bg_color = Color(1.0, 0.176, 0.333, 1.0)  # #FF2D55
	style.corner_radius_top_left = 2
	style.corner_radius_top_right = 2
	style.corner_radius_bottom_left = 2
	style.corner_radius_bottom_right = 2
	_exit_button.add_theme_stylebox_override("normal", style)
	_exit_button.add_theme_stylebox_override("hover", style)
	_exit_button.add_theme_stylebox_override("pressed", style)
	_exit_button.add_theme_color_override("font_color", Color(1, 1, 1, 1))
	_exit_button.add_theme_font_size_override("font_size", 8)
	var btn_font_8 = load("res://assets/fonts/fusion-pixel-8px-proportional-zh_hant.ttf")
	if btn_font_8:
		_exit_button.add_theme_font_override("font", btn_font_8)

	_exit_button.pressed.connect(Callable(self, "exit_build_mode"))
	canvas.add_child(_exit_button)
...
func _screen_to_tile(screen_pos: Vector2) -> Vector2i:
	## 將螢幕像素座標轉為格子座標。
	## 使用 Viewport 的 screen transform 的反矩陣，正確處理視窗縮放。
	## 假設地圖左上角對應 Viewport 座標 (0, 28)（頂部 HUD 28px）。
	var viewport: Viewport = get_viewport()
	var transform: Transform2D = viewport.get_screen_transform()
	var local_pos: Vector2 = transform.affine_inverse() * screen_pos
	var map_origin: Vector2 = Vector2(0, 28)
	local_pos -= map_origin
	return Vector2i(
